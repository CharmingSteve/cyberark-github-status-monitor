...
# First create the Slack integration
resource "statuscake_integration" "slack" {
  name             = "GitHub Status Slack Integration"
  integration_type = "Slack"
  
  # Slack integration specific settings
  slack_settings {
...
resource "statuscake_contact_group" "slack_alerts" {
  ...
  # Reference the integration ID
  integration_ids = [statuscake_integration.slack.id]
...
resource "statuscake_uptime_check" "github_status" {
  ...
  contact_ids  = [statuscake_contact_group.slack_alerts.id]
  regions      = ["US", "EU"]
  confirmation = 2
  trigger_rate = 5
